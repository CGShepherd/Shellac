...
if "DR-039 common post-EQ DC block" in text:
    print("replay_eq.py already contains DR-039; no change.")
else:
...
'''
    new = '''    # DR-039 common post-EQ DC block before the SCH107 filter/bypass split.
...
'''
    if old not in text:
        raise SystemExit("Expected SCH103 output block not found.")
    path.write_text(text.replace(old,new),encoding="utf-8")
    print("Patched replay_eq.py for DR-039")
