...
def build_unicase_fit_decision() -> UnicaseFitDecision:
    ...
    psu = VerifiedEnclosure(
        role="psu", manufacturer="METCASE", family="UNICASE 2",
        order_code="M5502119", colour="Black RAL 9005",
        external_width_mm=260.0, external_depth_mm=250.0, external_height_mm=90.2,
        base_pcb_width_mm=241.0, base_pcb_depth_mm=229.0,
        panel_thickness_mm=2.0,
    ...
    findings = [
        ...
        "G3-022 rejects M5501119; G3-023 freezes the next larger black UNICASE 2 M5502119 for the PSU using explicit geometric and passive-thermal reserve without inventing a temperature prediction.",
        "PSU M5502119 restores the historical >=80 mm internal-height screening gate and carries forward the exact SCHURTER KMF1.1121.11 mains-entry architecture.",
    ...
    return UnicaseFitDecision(
        ...
        audio_status=FitStatus.FROZEN, psu_status=FitStatus.FROZEN,
        findings=findings,
        open_items=[
            "verify PSU closed-box temperatures on the first powered prototype; this is a thermal-path verification rather than an enclosure-size selection blocker",
...
def validate_unicase_fit_decision(model: UnicaseFitDecision) -> list[str]:
    ...
    if model.psu.order_code != "M5502119" or model.psu_status is not FitStatus.FROZEN:
        issues.append("PSU enclosure must be frozen as black M5502119 after G3-023")
    if model.psu.usable_inside_height_mm is None or model.psu.usable_inside_height_mm < 80.0:
        issues.append("PSU enclosure does not preserve the historical 80 mm internal-height gate")
