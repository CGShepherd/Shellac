"""SCH109 AE-041 controls and user-interface builder."""
...
CONTROL_NETS={
    "SW901L":"BASS_L_SELECT","SW901R":"BASS_R_SELECT",
    "SW902L":"TREBLE_L_SELECT","SW902R":"TREBLE_R_SELECT",
    "SW903":"MODE_SELECT","SW904":"RUMBLE_BYPASS","SW905":"MUTE_CONTROL",
}
...
def add_controls(sheet):
    ...
    sheet.add_note("Bass/Treble are dual-mono; Rumble/Matrix/Mute are shared downstream controls.")
    positions=[
        Point(55,70),Point(55,115),Point(145,70),Point(145,115),
        Point(235,92),Point(315,70),Point(315,115)]
    for control,at in zip(CONTROLS,positions):
        c=sheet.add_component(_control_component(control,at))
        sheet.connect_pin_to_net(c,"CONTROL",CONTROL_NETS[control.identifier],stub_dy=-10.0)
    for i,(indicator,x) in enumerate(zip(INDICATORS,(175,250))):
        y=230
        led=sheet.add_component(_indicator_component(indicator,Point(x,y)))
        r=sheet.add_component(resistor(f"R90{6+i}",f"{LED_SERIES_RESISTANCE_OHM:g}",
        ...
        sheet.connect_points(rail_start,rail_pin); sheet.add_label(indicator.rail,rail_start.x,rail_start.y)
        sheet.connect_points(drive_pin,led_drive); sheet.connect_points(pin_position(tp,"TP"),led_drive)
        return_end=Point(led_return.x,led_return.y+10)
        sheet.connect_points(led_return,return_end); sheet.add_label("0VA",return_end.x,return_end.y)
